...
```mermaid
graph TD
    CLK_50[Clock de 50 MHz PIN_Y2] --> COUNTER[Contador de 20 bits Counter.bdf]
    CLK_50 --> PLL[ALTPLL PLL_27MHz]
    SIG_IN[Sinal da Corda OSC_INPUT] --> FSM[Unidade de Controle UnidadeControle.bdf]
    FSM -->|CLEAR| COUNTER
    FSM -->|LOAD| REG[Registrador DFF LPM_FF]
    COUNTER -->|Valor Atual 20-bit| REG
    REG -->|Período Estável 20-bit| COMP[Comparador ComparadorDeLeds.bdf]
    COMP -->|LEDs de status| LEDS[LEDs: mdup, dup, afinado, ddown, mddown]
    
    subgraph Bloco de Teste
        PLL -->|Clock Isolado 27.6363 MHz| OSC_GEN[Gerador de Oscilação geradorOsc.bdf]
...
## 3. Explicação Detalhada dos Módulos (Diagrama de Blocos - .bdf)

### A. Unidade de Controle (`UnidadeControle.bdf`)
...
### C. Registrador (`LPM_FF`)
Um registrador paralelo genérico de 20 bits instanciado via MegaWizard com:
...
### D. Comparador de LEDs (`ComparadorDeLeds.bdf`)
Contém a lógica de limiares (através de blocos `LPM_COMPARE` e constantes) para acender os 5 LEDs de status. Ele converte o número de ciclos de clock (período) em faixas de afinação:
...
### E. Gerador de Oscilação e PLL (`geradorOsc.bdf` e `PLL_27MHz`)
Este bloco serve como um **simulador** ou gerador de sinal de teste para a placa.
* Para atender a restrições específicas, foi criado um bloco **ALTPLL** (`PLL_27MHz`) que recebe o clock de 50 MHz da placa e entrega um clock perfeitamente isolado de **27.6363 MHz** para este simulador.
* O simulador pega esse clock de 27.6363 MHz e faz uma divisão de frequência para controlar a velocidade com que a "tarracha virtual" (LPM_COUNTER) se move ao segurarmos os botões `KEY0` e `KEY2`.
* Essa separação é genial porque o simulador gera a onda em 27.6363 MHz, mas o circuito de medição (`Counter.bdf`) continua rodando a 50 MHz. Isso garante que a matemática dos LEDs continue 100% precisa!

---

## 4. A Matemática da Afinação (Muito Importante para a Banca)
Os professores adoram perguntar **"de onde vieram estes números mágicos?"** no arquivo `ComparadorDeLeds.bdf`. Veja a demonstração passo a passo:
...
Agora, veja os limites configurados nos blocos `LPM_CONSTANT` do arquivo `ComparadorDeLeds.bdf` e as respectivas frequências equivalentes calculadas pela fórmula $f = \frac{50.000.000}{N}$:

| Sinal LED | Limite do Contador ($N$) no Bloco | Frequência Equivalente ($f$) | Significado Lógico |
| :--- | :--- | :--- | :--- |
...
* **Resposta:** Da forma que está estruturado fisicamente através dos comparadores (`LPM_COMPARE`) no arquivo `ComparadorDeLeds.bdf`, ele é calibrado de forma fixa e específica para a corda Mi grave (E2, ~82,41 Hz). Para suportar outras cordas usando esquemáticos puros, teríamos que multiplicar os blocos comparadores e usar lógicas de Multiplexadores (MUX) associados às chaves `SW` para selecionar qual bloco de comparação alimentar os LEDs dependendo da corda escolhida. A nossa implementação preferiu focar na excelência e precisão da nota Mi2.
