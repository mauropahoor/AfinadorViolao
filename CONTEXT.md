...
- **`selecao_corda` (`SW15-13`)**: Seleção da corda (000=E2, 001=A2, 010=D3, 011=G3, 100=B3, 101=E4).
...
## 📝 Histórico Recente de Modificações (Changelog para Apresentação)
As seguintes correções e implementações foram realizadas recentemente:
1. **Correção do Bug Matemático no Visor LCD:** Resolvido o erro de precisão de casas decimais que fazia o display dar um pulo incorreto e "voltar" o algarismo das centenas ao ultrapassar certas frequências (ex: 119.9 Hz exibindo 100.0 Hz). O divisor foi corrigido em `LcdController.vhd`.
2. **Atualização da Documentação de Chaves (SW15-13):** Todas as menções às chaves incorretas "SW17-15" nos códigos fonte e nas documentações (`README.md`, `CONTEXT.md` e `Explicação.md`) foram revisadas para refletir o mapeamento físico e lógico real, que sempre esteve no SW15, SW14 e SW13.
3. **Mapeamento Corrigido dos LEDs Vermelhos (LEDR):** O array de LEDs vermelhos (`osc_visualizer`), que oscilava caóticamente com os sinais simulados, foi reprogramado no VHDL Top-Level para acender de forma estática apenas os LEDs efetivamente utilizados: o `LEDR[0]` acompanha a chave de habilitação, e os `LEDR[15..13]` acompanham visualmente a seleção da corda correspondente.
4. **Migração do Simulador para o Pino B14 (Assíncrono):** Para simular com exatidão a assincronicidade do mundo real, o `GeradorOsci.vhd` foi desacoplado do clock principal de 50 MHz e alimentado pela entrada `PIN_B14` (clock de 27 MHz).
5. **Calibração Fina dos Limites do pino B14 (27 MHz):** Realizada calibração empírica dos limites base (`limite_base`) de todas as 6 notas no gerador para compensar com precisão o cristal de 27 MHz da placa DE2-115, fazendo com que todas as notas (E2, A2, D3, G3, B3, E4) iniciem perfeitamente cravadas em "Afinado" no LCD e nos LEDs. Também ajustado o gerador de tick de 1ms (`div_1ms`) para 27.000 ciclos.
